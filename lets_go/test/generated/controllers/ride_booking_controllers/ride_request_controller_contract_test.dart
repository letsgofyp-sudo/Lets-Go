...
void main() {
  const sourcePath = 'lib/controllers/ride_booking_controllers/ride_request_controller.dart';
  final source = File(sourcePath).readAsStringSync();

  group('controllers/ride_booking_controllers/ride_request_controller.dart contract', () {
    ...
    test('contains expected callable symbols', () {
      ...
      expect(RegExp(r'\bgetEstimatedArrivalTime\s*\(').hasMatch(source), isTrue);
      expect(RegExp(r'\bgetEstimatedDepartureTime\s*\(').hasMatch(source), isTrue);
      expect(RegExp(r'\bgetEstimatedDuration\s*\(').hasMatch(source), isTrue);
      expect(RegExp(r'\bgetEstimatedTimeRange\s*\(').hasMatch(source), isTrue);
      ...
      expect(RegExp(r'\bgetFormattedDuration\s*\(').hasMatch(source), isTrue);
      ...
      expect(RegExp(r'\bgetFullRouteDuration\s*\(').hasMatch(source), isTrue);
      expect(RegExp(r'\bgetFullRoutePricePerSeat\s*\(').hasMatch(source), isTrue);
      ...
      expect(RegExp(r'\bgetSelectedRouteDuration\s*\(').hasMatch(source), isTrue);
