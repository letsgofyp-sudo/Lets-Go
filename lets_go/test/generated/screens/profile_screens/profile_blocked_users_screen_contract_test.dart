...
void main() {
  const sourcePath = 'lib/screens/profile_screens/profile_blocked_users_screen.dart';
  final source = File(sourcePath).readAsStringSync();

  group('screens/profile_screens/profile_blocked_users_screen.dart contract', () {
    test('source file is non-empty', () {
      expect(source.trim().isNotEmpty, isTrue);
    });
    test('declares expected classes', () {
      expect(source.contains('class ProfileBlockedUsersScreen'), isTrue);
      expect(source.contains('class _ProfileBlockedUsersScreenState'), isTrue);
    });
