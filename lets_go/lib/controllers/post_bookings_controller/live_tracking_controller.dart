...
class LiveTrackingController {
  ...
  Future<void> init() async {
    debugPrint(
      '[LiveTracking] init: tripId=$tripId currentUserId=$currentUserId isDriver=$isDriver bookingId=$bookingId',
    );
  ...
  Future<void> startRide() async {
    try {
      debugPrint(
        '[LiveTracking] startRide called (isDriver=$isDriver, tripId=$tripId, bookingId=$bookingId)',
      );
  ...
  Future<void> _sendLocationUpdate(Position pos) async {
    try {
      ...
      try {
        ...
        if (prev != null) {
          toSend.addAll(
            MapUtil.densifyBetween(prev, current, maxStepMeters: 25),
          );
        } else {
          toSend.add(current);
        }
  ...
  Future<void> _flushOfflineQueueIfAny() async {
    ...
    try {
      ...
      for (final it in batch) {
        ...
        final speed = sRaw is num
            ? sRaw.toDouble()
            : double.tryParse(sRaw?.toString() ?? '');
  ...
  Future<void> _loadTripLayout() async {
    try {
      debugPrint(
        '[LiveTracking] _loadTripLayout: fetching layout for tripId=$tripId',
      );
      ...
      debugPrint(
        '[LiveTracking] _loadTripLayout: routeStops=${routeStops.length}',
      );
      ...
      debugPrint(
        '[LiveTracking] _loadTripLayout: routePolyline points=${routePolyline.length}',
      );

      // Bookings / pickup & dropoff points

      final rawBookingList =
          (data['bookings'] ?? data['passengers'] ?? data['trip_bookings']);
      ...
      if (isDriver &&
          ...
          confirmedPassengerBookings.isNotEmpty) {
        final firstId = _asInt(
          confirmedPassengerBookings.first['booking_id'] ??
              confirmedPassengerBookings.first['id'],
        );
      ...
      for (final b in bookings) {
        ...
        if (!isDriver && bookingId != null) {
          ...
          final bool isCurrentBooking =
              (id != null && id == bookingId) ||
              (passengerFromStopOrder == null &&
                  passengerUserId != null &&
                  passengerUserId == currentUserId);
    ...
    _pollTimer = Timer.periodic(const Duration(seconds: 3), (_) async {
      try {
        ...
        if (data['success'] == true) {
          ...
          if (!isDriver) {
            if (bookingRideStatus == 'DROPPED_OFF' ||
                bookingRideStatus == 'DROPPED_EARLY') {
              rideStarted = false;
          ...
          debugPrint(
            '[LiveTracking] poll: driver=$driver passengers=${list.length}',
          );
          ...
          try {
            ...
            driverEtaSecondsToFinal = _asIntSafe(
              runtime?['driver_eta_seconds_to_final'],
            );

            passengerEtaSecondsToDropoff = _asIntSafe(
              runtime?['passenger_eta_seconds_to_dropoff'],
            );
          } catch (_) {
            driverSpeedKph = null;
          ...
          try {
            ...
            driverLastSeenSeconds = _asIntSafe(
              driverMeta?['last_seen_seconds'],
            );
          } catch (_) {
  ...
  Future<void> generatePickupCode() async {
    ...
    if (!isDriver || targetBookingId == null) {
      debugPrint(
        '[LiveTracking] generatePickupCode called in invalid context (isDriver=$isDriver, bookingId=$bookingId)',
      );
    ...
    try {
      ...
      debugPrint(
        '[LiveTracking] generatePickupCode: tripId=$tripId bookingId=$bookingId driverId=$currentUserId lat=${pos?.latitude} lng=${pos?.longitude}',
      );
      ...
      if (res['success'] == true) {
        ...
        pickupMaxAttempts = res['max_attempts'] is int
            ? res['max_attempts'] as int
            : null;
        ...
        errorMessage =
            res['error']?.toString() ?? 'Failed to generate pickup code';
  ...
  Future<void> verifyPickupCode(String code) async {
    if (isDriver || bookingId == null) {
      debugPrint(
        '[LiveTracking] verifyPickupCode called in invalid context (isDriver=$isDriver, bookingId=$bookingId)',
      );
    ...
    try {
      ...
      if (res['success'] == true) {
        ...
        errorMessage =
            res['error']?.toString() ?? 'Pickup code verification failed';

        debugPrint(
          '[LiveTracking] verifyPickupCode error: $errorMessage remaining=${res['remaining_attempts']}',
        );
