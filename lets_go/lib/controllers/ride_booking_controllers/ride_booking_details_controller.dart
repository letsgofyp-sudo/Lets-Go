...
class RideBookingDetailsController {
  ...
  Future<void> loadRideDetails(String tripId) async {
    try {
      ...
      debugPrint('DEBUG: RideBookingDetailsController - API response keys: ${data.keys.toList()}');
      debugPrint('DEBUG: RideBookingDetailsController - trip data: ${data['trip']}');
      if (data['trip'] != null) {
        debugPrint('DEBUG: RideBookingDetailsController - base_fare from API: ${data['trip']['base_fare']}');
        debugPrint('DEBUG: RideBookingDetailsController - is_negotiable from API: ${data['trip']['is_negotiable']}');
      }
      
      if (data['success'] == true) {
        rideData = data;
        debugPrint('DEBUG: RideBookingDetailsController - rideData stored successfully');
  ...
  Future<void> _recalculateFareAsync() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        final durationMinutes = response['fare_calculation']['total_duration_minutes'] as int?;
        if (durationMinutes != null) {
          selectedRouteDuration = durationMinutes;
          debugPrint('Dynamic fare and duration recalculated: ${response['fare_calculation']['total_fare']} for stops $selectedFromStop-$selectedToStop, seats: $numberOfSeats, duration: ${durationMinutes}min');
        } else {
          debugPrint('Dynamic fare recalculated: ${response['fare_calculation']['total_fare']} for stops $selectedFromStop-$selectedToStop, seats: $numberOfSeats (no duration data)');
        }
  ...
  double calculateTotalFare() {
    ...
    if (dynamicFare != null) {
      ...
      debugPrint('Using dynamic fare: $totalFare for stops $selectedFromStop-$selectedToStop, seats: $numberOfSeats');
      return totalFare;
    }
    ...
    if (stopBreakdownData != null && stopBreakdownData.isNotEmpty) {
      ...
      double bulkDiscountPercentage = fareCalculation?['bulk_discount_percentage'] ?? 0.0;
      ...
      debugPrint('Manual fare calculated: $finalFare for stops $selectedFromStop-$selectedToStop, seats: $numberOfSeats');
      return finalFare;
    }
    
    // Fallback to base fare calculation
    final baseFare = (rideData['trip']?['base_fare'] as num?)?.toDouble() ?? 0.0;
    debugPrint('Using fallback fare calculation: $baseFare * $numberOfSeats = ${baseFare * numberOfSeats}');
  ...
  String plateNumber() {
    final v = getVehicleInfo();
    final cand = [v['plate_number'], v['plate_no'], v['license_plate'], v['plate'], v['number_plate']];
    for (final c in cand) {
      final s = c?.toString();
  ...
  double getPricePerSeat() {
    ...
    final dynamicFare = rideData['trip']?['dynamic_fare'];
    if (dynamicFare != null) {
      final baseFarePerSeat = (dynamicFare['base_fare_per_seat'] as num?)?.toDouble() ?? 0.0;
      debugPrint('DEBUG: RideBookingDetailsController.getPricePerSeat() - using dynamic fare: $baseFarePerSeat');
      return baseFarePerSeat;
    }
    
    final bookingInfo = getBookingInfo();
    final pricePerSeat = (bookingInfo['price_per_seat'] as num?)?.toDouble() ?? 0.0;
    debugPrint('DEBUG: RideBookingDetailsController.getPricePerSeat() - booking_info: $bookingInfo');
    debugPrint('DEBUG: RideBookingDetailsController.getPricePerSeat() - returning: $pricePerSeat');
    return pricePerSeat;
  }
  ...
  String getFormattedDepartureTime() {
    ...
    try {
      ...
      final departureDateTime = DateTime(date.year, date.month, date.day, hour, minute);
