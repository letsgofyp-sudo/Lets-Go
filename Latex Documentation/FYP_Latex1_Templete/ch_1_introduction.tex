...
The tangible Outputs of the Lets Go Ride Sharing App mobile application project are software modules, artefacts of documentation and implementation deliverables.

\textbf{List of Deliverables:}

\begin{enumerate}
    \item \textbf{Registration \& Verification Module} – A complete system to register users (Drivers \& Passengers). This module collects and verifies this user data. Name /Person name, Phone number, e-mail, Post office address, CNIC, driving license, Vehicle registration details. In addition to capability to swap drivers and passengers, they have a built-in two enigma authentication system.
...
\item \textbf{Internet Connectivity \& Location Accuracy:} Real-time features like ride matching, live location tracking, and fare estimates require reliable internet connections and accurate GPS from the vehicles. If network connectivity is poor or the service is inaccurate, access, performance and delays to critical features of the application may be limiting.

\item \textbf{Comprehensive manual Security and Verification Overheads:} Manual verification of crucial paperwork like CNICs, driving licenses, and vehicle registration forms enhances platform trust and security. This is beneficial to ensure the sense of authenticity for users, but it could also extend the process to onboard them, potentially dampening the user experience for new users when their accounts are activated.

\item \textbf{Automated Payment Gateways:} Use of integrated online payment gateways of the web is not part of the current implementation. The system is, therefore, cash based and based on 'verification' of bank transfers. This can cause a timelapse in receiving confirmation on the payment and increase in administrative work for the management of transactions and reconciliation.

\item \textbf{Geographic Scope:} At the time of initial deployment, the application may be limited to certain local communities, rural areas and limited urban areas. By limiting these coverages, it decreases the services available and users' access in other locations.

\item \textbf{Resource and Time Constraints:} With all the project being built in an academic setting within financial and time constraints, some of the more complex features and large scale testing of the project may be delayed for a later release. Furthermore, there is a limit to the total development capacity and specialization in the project due to a development team comprising of two students.
\end{itemize}
...
A tech stack with optimal compatibility, processing speed, data reliability and longevity is carefully selected in the development of the Lets Go Ride Sharing App. Each technology has been selected for meeting certain project needs and with future expansion and maintainability in mind.

\textbf{For cross-platform mobile app development, Frontend Development:} Flutter together with the Dart SDK has been chosen for frontend application development and cross-platform mobile application development. With this structure, they can work with the same codebase from Android and iOS, which will help them to save on development time and with this ensure that their device will work on the same level of performance. In addition, the wide-ranging selection of Flutter widgets guarantees the flexibility needed to create a user-friendly and responsive interface that suits the project's goals and requirements.
