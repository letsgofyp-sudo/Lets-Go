...
The software design of the Lets Go Ride Sharing App and The Lets Go Ride Sharing App Recommendation System are detailed in this chapter. Renders the requirements identified in Chapter 3 into structured models, both architectural and behavioral and data. The aim of this chapter is to give a concrete idea of how to build the system scale-able, maintainable and modular.

The design adheres to Object-Oriented Design (OOD) and the architecture is a hybrid of Multi-Tier design and Model-View-Controller (MVC). This chapter includes architectural design, UML models (class, sequence, state, activity diagrams), data flow models, database design, and thorough algorithms for the core capabilities like the ride recommendation algorithm (based on machine learning), the route ETA calculation, and the SOS escalation policy.
...
The design process that has been used in the Lets Go Ride Sharing App system is Object-Oriented Design (OOD). The system has various real-world objects including Passenger, Driver, Ride, Vehicle, Booking, Payment, and Route with their own attributes and behaviors. These concepts are most easily represented in the software by creating software objects that represent them in the real world.
...
\begin{itemize}
    ...
    \item \textbf{Reusability and Extensibility:} The application consists of several modules (Authentication, Ride Matches, ML Recommendations, SOS Handling, Chat, Notifications) to achieve reuse through inheritance, interfaces and composition, OOD. New features (such as corporate rides, scheduled rides) can be added and core modules need not be rewritten.
    \item \textbf{Encapsulation and Modularity:} Using the MVC (MTV) pattern in Django. The models, views, controllers and services are separate modules, where each module has a specific function.
    \item \textbf{Multi-Tier Architecture:} The application is a multi-tier application (Client, Backend Servers, Database, External Services) and object-oriented modules can be distributed across the tiers.
    \item \textbf{Scalability Support:} Multiple backend servers for horizontal scaling of same OOP based services. These classes such as BookingManager, RideManager, VerificationManager and MLService split up complicated logic into small manageable pieces.
    \item \textbf{Simplified Maintenance:} Classes provide a way to separate complicated logic into manageable pieces which are easier to debug and update.
\end{itemize}

It takes into account good software engineering principles such as SOLID (Single Responsibility, Open-Closed, Liskov Substitution, Interface Segregation, Dependency Inversion), DRY (Don't Repeat Yourself), and Separation of Concerns.

\subsection{Software Process Model}

The project is built using the \textbf{Agile Incremental Development Model (AIDM)} using Scrum like iterations.


\subsubsection{Justification for Using Agile/Scrum}

\begin{itemize}
    \item \textbf{Rapid Requirement Evolution:} Ride-sharing systems need to evolve their requirements including new safety regulations, new verification flows, etc., and improvements in rider/driver matching. Agile allows the team to be stable, while still being able to adapt to change on a regular basis.
    
    \item \textbf{Modular and Incremental Delivery:} The system has been developed incrementally with a number of large increments making it very easy to do this.
    \begin{itemize}
    ...
    \end{itemize}
    This is a perfect incremental approach.
    
    \item \textbf{Continuous End-User Feedback:} Ride sharing companies need testing with real time users to test the usability, mapping accuracy, and effectiveness of ride matching. User feedback from early releases were fed into further developments in the agile development cycles.
    
    \item \textbf{Concurrent Development of Subsystems:} Multiple modules (Mobile App, Backend API, External Services, Chatbot, ML Recommendation Engine) have been developed concurrently, through iterative sprints.
    
    \item \textbf{Reduced Project Risk:} Minimized the risk for last minute failures by developing features, building them, and testing them in bursts.
    
    \item \textbf{Fit for Academic Projects:} Structured artifacts (SRS, SDD, UML models) are possible in Fit for Academic Projects: Agile, but it is flexible enough to accommodate documentation in other ways.
\end{itemize}

Testing and validation was incorporated in each sprint as a part of the continuous integration, unit testing, and user acceptance testing at the end of every increment.

%===========================================================
\section{System Overview}
%===========================================================

This section gives a high level description of system components and interactions with external entities.

\subsection{External Actors}

\begin{itemize}
    \item \textbf{Passenger (Mobile App):} Orders and pays for rides, reviews available options, monitors drivers, and finishes rides.
    \item \textbf{Driver (Mobile App):} Is available, accepts a ride, provides real-time location, and completes trips.
    \item \textbf{Administrator (Web Portal):} Manages disputes, verifies users, and monitors the system.
\end{itemize}

\subsection{External Services}

\begin{itemize}
    \item \textbf{Map API (OpenStreetMap/OSRM):} Way to calculate route, to track route, and to calculate estimated time of arrival calculations.
    \item \textbf{Notification Services (Supabase/FCM):} These services process notifications related to ride requests, ride acceptances, and SOS notifications.
    \item \textbf{Machine Learning Service:} Recommends the appropriate ride to passengers, based on past data and user preferences.
    \item \textbf{Generative AI/Chatbot Assistant:} Improves content quality, relevance, engagement, and helpfulness using AI-driven interactions.
    \item \textbf{Storage Service:} Storage of profile pictures, driver's license and verification documents.
\end{itemize}

\subsection{Backend (Django MVC + JSON APIs)}

Back-end is done with Django MVC framework and communicates with the outside with JSON APIs. Core responsibilities include:
\begin{itemize}
    \item Ride matching operations
    \item Ticketing systems
    \item Apply periodic location updates using HTTP requests.Use periodic location updates via http requests.
    \item A verification image is needed in conjunction with profile.Along with profile, a verification image is needed.
    \item Machine-learning recommendation integration
    \item Begin escalation and disputes.Start escalation and dispute process.
    \item All activities and events relating to the process of communicating: between the driver and passengers, between passengers
\end{itemize}
...
\caption{Context Diagram: System boundary and interactions with external actors and services}
\label{fig:context}
\end{figure}

The Lets Go Ride Sharing App system boundary diagram shows the interactions between the system and external entities such as passengers, drivers, and administrators along with interaction with third-party services like the Map API, Notification Service, Machine Learning Service, Chatbot Service and Storage Service. Data flows are ride requests, ride update, notifications and document exchanges.

%===========================================================
\section{Architectural Design}
%===========================================================

The architectural style of the Lets Go Ride Sharing App is a hybrid of the Multi-Tier Architecture pattern for the overall structure and Model-View-Controller (MVC) pattern for the Django backend. This mix of features results in separation of concerns, scalability, maintainability and support for distributed deployments over multiple servers.
...
Backend servers communicate with external services using RESTful API secured via token based authentication with fault-tolerant retry infrastructure.
...
\subsubsection{Notification Service Responsibilities}
\begin{itemize}
    \item Send push notifications with Firebase Cloud Messaging (FCM)
    \item This feature indicates that an alert for ride requests, driver acceptance and SOS events will be sent.
\end{itemize}

\subsubsection{Responsibilities of Machine Learning Recommendation Service}
\begin{itemize}
    \item Reads request data from motorcycle(s)
    \item Suggests suitable ride methods
    \item Performs TF-IDF similarity techniques in the generation of recommendations
\end{itemize}

\subsubsection{Chatbot Service Responsibilities}
\begin{itemize}
    \item Automatically answers common questions
\end{itemize}

\subsubsection{Storage Service Responsibilities:}
\begin{itemize}
    \item Uploading profile photographs
    \item Requires uploading of verification and identity documents.Requires verification and identity documents to be uploaded.
    \item Creating signed URLs for secure file access.
\end{itemize}

\subsection{Architecture Diagram}

The architecture diagram shows the four major layers in the system – the Client Layer, Application Layer, External Services Layer, and the Data Layer. The arrows in the diagram are used to show the flow of data and the communication between the APIs in each layer.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth,keepaspectratio,height=0.8\textheight]{assets/architecture.jpg}
\caption{Multi-tier system architecture diagram depicting interactions and deployments of the system components.}
...
This section will present the detailed design models for the system. These models represent behaviour, structure and interactions of a system in UML diagrams. They help developers to learn about processes and the sytem's capabilities prior to implementation.

%===========================================================
\subsection{Activity Diagrams}

Activity diagrams show the operations of various modules of the system. There are various activities diagrammatically shown in each primary module, which indicates interaction with the users, decision points, checkpoints, interaction with the database, interaction with the model.

\subsubsection{The first phase is Authentication – Onboarding (Registration, OTP Verification, and Admin Approval).The initial phase is Authentication – Onboarding (Registration, OTP Verification and Admin Approval)}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth,keepaspectratio,height=0.8\textheight]{assets/activity diagrams/activity_auth.jpg}
\caption{A workflow that allows both the authentication and onboarding of activity.}
\label{fig:activity_auth}
\end{figure}

The complete registration and verification process as shown in this activity diagram. The users fill in details, upload necessary files, get a one-time password by email or SMS, confirm OTP and await admin approval. Administrators look at the documents that have been submitted and can determine if it is accurate or not, whether they want it submitted, or if they think they shouldn't be submitted.

\clearpage

\subsubsection{Data is uploaded onto computers for documentation purposes and verified.}
...
This diagram illustrates the document submission, document size and format validation, secure storage, document administration steps (approvals and reject steps).
...
\caption{Now, let's create an interactive diagram that illustrates the workflow for managing a vehicle.}
\label{fig:activity_vehicle}
\end{figure}

This diagram shows steps drivers take to add a vehicle to the system, upload vehicle images, pass through system checks and receive an optional administrative review for any large changes.
...
\caption{Let's create a sequence diagram for the User Authentication (signup and login) flow.}
\label{fig:activity_ride_mgmt}
\end{figure}

This sequence diagram (SDG) shows the flow of sign-up and sign-in. The user provides information to the application during the application's registration process, and the application passes these to the back-end. The backend verifies the information, adds the user account and pushes an OTP via notification service back to the gateway and checks the OTP, and puts the status "pending administrative verification". To log in, the user sends user credentials, back-end checks these credentials, verifies account and returns a JWT token of authentication.
...
\textbf{Description:} This sequence diagram (SDG) shows the flow of sign-up and sign-in. The user provides information to the application during the application's registration process, and the application passes these to the back-end. The backend verifies the information, adds the user account and pushes an OTP via notification service back to the gateway and checks the OTP, and puts the status "pending administrative verification". To log in, the user sends user credentials, back-end checks these credentials, verifies account and returns a JWT token of authentication.
...
\caption{To create a sequence diagram to represent a driver posting a ride.}
\label{fig:seq_ride_post}
\end{figure}

\textbf{Description:} This diagram shows how a driver forms a ride. The driver brings up the map interface, chooses stops from a map service that performs geocoding, provides ride details, and submits the ride. It then validates the information, passes it to a fare calculation component to calculate fare, updates the database with ride information, and indicates that it was created successfully. Then the ride is shown in the ride marketplace.
...
\textbf{Guidelines:}
\begin{itemize}
    \item Make sure that all entities (tables/collections) are clearly identified.
    \item Highlight Primary Keys (PK).
    \item Highlight Foreign Keys (FK).
    \item Show relationship cardinality (1:1, 1:N, M:N).
    \item Consistency with the previous database tables.
\end{itemize}

\textbf{Include in ERD:}
\begin{itemize}
    \item \textbf{User Entity}: User entity includes: User (abstract superclass), Passenger, Driver.
...
The database schema is normalized to 3NF to eliminate Data redundancy. his structured
Design guarantees that data is consistent and compliant by enforcing strict relationships (Foreign Keys) between data elements.
Relationships, such as Users to Rides and Users to Bookings, are presented in tables. It has efficient querying capabilities via indexing on
Commonly used fields such as user_id and ride_id. In the modularity of the schema, the following features were observed:
scalability - ability to extend tables seamlessly without impacting core operations. Finally,
Security is ensured by separating key authentication information in the User table from sensitive information.
Based on public operational data.

\subsection{Data Dictionary}

The structure of all the primary data elements in the database is defined in the Data Dictionary

\textbf{Guidelines:}
\begin{itemize}
    \item Include fieldname,Datatype,Size,Description,Constraints.
\end{itemize}
...
The User table stores authentication credentials and user profile information. It supports permission and authentication based on the role of the user.

\textbf{Description:}
\begin{itemize}
    \item \textbf{Relationships:} Tied to transactions, predictions or system logs.
...
This subsection describes domain-specific core data tables. These structures are used for the main storage. Data that is processed by the Ride Booking system.
...
The following measures are taken to help ensure that the data stored is confidential and secure:
\begin{itemize}
    \item Password Encryption using secure hashing algorithms (e.g., bcrypt, SHA-256 with salt).
    \item Role Based Access Control (RBAC): To limit access between passengers, drivers, and admins.
    \item Validation and constraint checking at the database-level (such as NOT NULL, UNIQUE constraints).
    \item Ensure communication to endpoints which access the DB is done securely via HTTPS.
    \item Back up and disaster recovery plan for the database.
\end{itemize}

\subsection{Scalability Considerations}

The database is designed to be scalable, allowing for future expansion of the ride-sharing service.
\begin{itemize}
    \item Scaling across systems horizontally with distributed database systems.
    \item Indexing on frequently accessed attributes (such as \texttt{pickup\_location}, \texttt{drop\_location}, \texttt{status}) to enhance the performance of searches.
    \item Optimizing data for frequently-used operations such as active ride and booking summaries.
    \item Load balancing of API requests on different database instances.
    \item Sharding or partitioning of data in large environments (e.g., sharding old ride records by year).
\end{itemize}
...
The software architecture and design of Let's go was discussed in detail in the above chapter. It described the evolution from requirements to a real system blueprint in an Agile incremental model. 

Key modeling diagrams such as Activity, Class, Sequence, State Transition. The dynamic and structural behaviour of the system was illustrated by the use of diagrams. Data the flow of data was shown through the different aspects of operation using Flow Diagrams (DFDs), for riding tracking purposes and dispute resolution.

The chapter ended with a detailed design of the data section of PostgreSQL. Relationships, ER diagrams, data dictionaries, and schemas for scalable, secure database relationships, and normalized data structure.