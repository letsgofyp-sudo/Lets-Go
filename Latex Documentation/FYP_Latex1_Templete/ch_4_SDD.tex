...
The software design for Lets Go Ride Sharing App ride sharing and recommendation system is given in detail in this chapter. Transforms the needs of Chapter 3 into structured architectural, behavioural and data models. The intention of this chapter is to act as a compass on a clear blueprint to implement the system in a maintainable, scalable and modular manner.
...
\begin{itemize}
    ...
    \item \textbf{Modular \& Incremental Delivery:} The system was built in major increments:
    \begin{itemize}
    ...
    \end{itemize}
    This is a perfect fit with the incremental approach!
    
    \item \textbf{End Users Ongoing Feedback:} Ride-sharing applications must be piloted by real users to ensure usability, map accuracy and ride matching quality. Agile cycles enabled feedback from users on early releases.
...
\begin{itemize}
    \item \textbf{Map API (OpenStreetMap/OSRM):} For route calculation, back tracking and ETA estimation.
    \item \textbf{Notification Services (Supabase/FCM):} Delivers real-time notifications to users on ride requests, acceptances, and SOS alerts.
    \item \textbf{Machine Learning Service:} Suggests the most suitable rides for passengers according to past information.
    \item \textbf{Generative AI / Chatbot Assistant:} Improves content production efficiency and quality, such as relevance and engagement, with the help of Generative AI / Chatbot Assistant.
    \item \textbf{Storage Service:} Stores profile images, license files and documents for verification.
\end{itemize}

\subsection{Backend (Django MVC + JSON APIs)}

The backend development is done in Django MVC, with JSON APIs. The central component of the system fulfills:
\begin{itemize}
    \item Ride matching logic
    \item Booking and cancellation
    \item Periodic location updates by HTTP requests
    \item Verify images and add to profile.
...
The context diagram depicts the boundary of the Lets Go Ride Sharing App system and its relationships with external entities such as Passengers, Drivers, Administrators, and third-party services (Map API, Notification Service, ML Service, Chatbot, Storage). Data flows represent ride requests, updates, notifications, and document transfers.

%===========================================================
\section{Architectural Design}
%===========================================================

The Lets Go Ride Sharing App system is designed in a hybrid architecture with two broad paradigms: 1. Multi-Tier Architecture for the overall system structure and 2. Model-View-Controller (MVC) internally in the backend through Django framework. The combination offers separation of concerns, scalability, maintainability, and distributed deployment over several servers.
