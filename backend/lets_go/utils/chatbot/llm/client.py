...
def _summary_prompt(previous_summary: str, history: list[dict]) -> str:
    lines = [
        'You update a running conversation summary for a ride-sharing app chatbot.',
        'Goal: keep a compact summary that helps future responses.',
        'Hard rules:',
        '- Do NOT include or store sensitive data: CNIC, driving license numbers, bank account numbers, IBAN, phone numbers, emails, passwords, tokens.',
        '- Do NOT include URLs or images.',
        '- Keep it short: max 10 bullet points.',
        '- Prefer stable user preferences and recent intents over raw logs.',
        '',
        'Previous summary:',
    ...
    lines.append('')
    lines.append('Output ONLY the updated summary text.')
    return "\n".join(lines)


def llm_summarize_history(st: ConversationState) -> Optional[str]:
    prov = llm_provider()
    ...
    try:
        ...
        prompt = _summary_prompt(str(getattr(st, 'summary', '') or ''), hist)
        payload = {
            'model': cloud_model(),
            'messages': [
                {'role': 'system', 'content': 'You write safe conversation summaries.'},
        ...
        if llm_debug_enabled():
            logger.debug("[chatbot][llm][summary][openai_compat] POST %s model=%s", url, payload.get('model'))
...
def _plan_prompt(
    ...
    *,
    ...
    lines = [
        ...
        state_text or '',
        '',
        'Conversation summary (may be incomplete):',
...
def llm_plan(st: ConversationState, text: str, *, tools_text: str) -> Optional[dict]:
    ...
    try:
        ...
        hist = _minimized_history(st.history or [])
        state_txt = (
            f"last_trip_id={str(st.last_trip_id or '')} "
            f"last_booking_id={str(st.last_booking_id or '')} "
            f"active_flow={str(st.active_flow or '')} "
            f"awaiting_field={str(st.awaiting_field or '')}"
        )
        ...
        prompt = _plan_prompt(
            ...
            summary_text=str(getattr(st, 'summary', '') or ''),
        )
        payload = {
            'model': cloud_model(),
            'messages': [
...
def _chat_prompt(history: list[dict], text: str, *, summary_text: str) -> str:
    lines = [
...
def llm_chat_reply(st: ConversationState, text: str, *, brain_mode_override: Optional[bool] = None) -> Optional[str]:
    ...
    prov = llm_provider()
    prompt = _chat_prompt(st.history or [], text, summary_text=str(getattr(st, 'summary', '') or ''))
