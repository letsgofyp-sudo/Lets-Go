...
from ..core import ORS_API_KEY, STOPS_GEO_JSON
...
def geocode_stop_candidates(query: str, country: str = "Pakistan", *, limit: int = 5) -> List[Dict[str, Any]]:
    ...
    major_cities = {
        ...
        "bahawalpur": (29.3956, 71.6833, "Bahawalpur, Pakistan"),
        "vehari": (30.0333, 72.3333, "Vehari, Pakistan"),
        "peoples colony": (30.0500, 72.3500, "Peoples Colony, Vehari"),
    }
    key = normalize_text(query)
    canonical = major_cities.get(key)
    if canonical:
        lat, lon, name = canonical
        out.append({"lat": lat, "lon": lon, "display_name": name, "address": {}, "source": "major_city"})

    # Data quality guard: local stops data can contain incorrect coordinates for common city names.
    # If a query is a canonical major city and the local-stops match is very far away, reject it.
    if canonical and local:
        try:
            canonical_lat, canonical_lon, _ = canonical
            local_dist_km = haversine_distance(
                float(local.get('lat')),
                float(local.get('lon')),
                float(canonical_lat),
                float(canonical_lon),
            )
            if local_dist_km > 150.0:
                trace(
                    'osm_client.geocode_stop_candidates.local_rejected_far_from_canonical',
                    query=query,
                    local_display_name=str(local.get('display_name') or ''),
                    local_dist_km=float(local_dist_km),
                )
                out = [c for c in out if c.get('source') != 'local']
        except Exception as e:
            trace(
                'osm_client.geocode_stop_candidates.local_canonical_guard_error',
                query=query,
                error=str(e),
            )
    ...
    for c in out:
        try:
            k = (
                round(float(c.get('lat')), 6),
                round(float(c.get('lon')), 6),
                normalize_text(str(c.get('display_name') or '')),
            )
        except Exception:
            continue
        if k in seen:
...
def osrm_route(coordinates: List[Tuple[float, float]], overview: str = "simplified") -> Optional[Dict[str, Any]]:
    """
...
def ors_directions(coordinates: List[Tuple[float, float]]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        trace(
            'osm_client.ors_directions.exit',
...
def build_route_stops(stop_names: List[str]) -> List[Dict[str, Any]]:
    ...
    for i, name in enumerate(stop_names):
        ...
        if not geo:
            raise ValueError(f"I couldn't find a matching location for '{name}'. Please try a more specific place name (city/area).")
        trace(
            'osm_client.build_route_stops.geocoded',
            name=name,
    ...
    if route and route.get("distance_km") and route.get("duration_seconds") is not None:
        trace(
            'osm_client.build_route_stops.routed',
        ...
        total_osrm_dur = float(route["duration_seconds"]) / 60.0
        haversine_distances = []
        ...
        for i, stop in enumerate(route_stops):
            if i == 0:
                ...
                stop["duration_from_previous_minutes"] = (dist / 50.0) * 60.0
