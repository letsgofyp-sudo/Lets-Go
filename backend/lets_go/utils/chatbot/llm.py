...
def llm_chat_reply(st: ConversationState, text: str) -> Optional[str]:
    if not llm_chat_enabled() and not llm_brain_mode():
        return None
...
def llm_rewrite_reply(st: ConversationState, user_text: str, draft_reply: str) -> Optional[str]:
    if not llm_chat_enabled() and not llm_brain_mode():
        return None
