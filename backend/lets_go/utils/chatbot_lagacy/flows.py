...
def render_location_choice(query: str, candidates: list[dict]) -> str:
    q = (query or '').strip()
    lines = [f"I found multiple matches for '{q}'. Please reply with the number you mean:"]
...
def list_user_vehicles(ctx: BotContext, *, limit: int = 20) -> str:
    ...
    if status in {401, 403}:
        return _format_api_result(status, out)
...
def list_user_created_trips_state(st: ConversationState, *, limit: int = 20, vehicle_id: Optional[int] = None) -> str:
    ...
    def _stops_from_trip_detail(trip_id: str) -> tuple[Optional[str], Optional[str]]:
        try:
            # In stateless regression/fuzzing, avoid N+1 calls that can slow down or hang the suite.
...
def continue_misc_flows(st: ConversationState, text: str) -> Optional[str]:
    ...
    if st.active_flow == 'chat_list' and st.awaiting_field == 'trip_id':
        # Align with regression expectations: only VERIFIED users can access chat list.
        ...
        if not trip_id:
            return 'Which trip chat do you want to view? Provide trip_id.'
        st.awaiting_field = None
    ...
    if st.active_flow == 'payment_details' and st.awaiting_field == 'booking_id':
        # Align with regression expectations: only VERIFIED users can access payment details.
        _, err = api.require_system_access(st.ctx)
        if err:
            reset_flow(st)
            return err
        bid = extract_booking_id(text) or to_int(text)
        if not bid:
            return 'Which booking payment details do you want? Provide booking_id.'
        st.awaiting_field = None
