        ...
        return JsonResponse(
            {
                ...
                'fields': getattr(ve, 'message_dict', None) or {'__all__': ve.messages},
            },
            status=400,
        )
    except Exception as e:
...
def user_profile(request, user_id):
    ...
    try:
        ...
        logger.exception('[user_profile] OperationalError while loading user data: %s', repr(e))
...
def user_emergency_contact(request, user_id):
    try:
        ...
        phone_no_raw = (payload.get('phone_no') or '').strip()
            ...
            return JsonResponse(
                {
                    'success': False,
                    'error': 'Validation failed',
                    'fields': getattr(e, 'message_dict', None) or {'__all__': e.messages},
                },
                status=400,
            )
...
def user_image(request, user_id, image_field):
    ...
    *_url fields. This view is now a best-effort fallback for old records
    ...
    """
    try:
        logger.debug("[user_image] Attempting to serve %s for user %s", image_field, user_id)
        legacy_fields = {
            'profile_photo',
            'live_photo',
            'cnic_front_image',
        ...
        if image_field not in legacy_fields:
            raise Http404("Invalid image field")

        if not hasattr(UsersData, image_field):
            url_field = f"{image_field}_url"
            if hasattr(UsersData, url_field):
                url = UsersData.objects.only(url_field).values_list(url_field, flat=True).get(id=user_id)
                if url:
                    return redirect(url)
            raise Http404("Invalid image field")
        ...
        try:
            ...
            logger.debug("[user_image] Warning: could not set local statement_timeout: %s", str(e))
        ...
        logger.debug("[user_image] Image data loaded")
        ...
        logger.debug("[user_image] Serving image content_type=%s", content_type)
        ...
        logger.exception("[user_image] Error serving image %s for user %s: %s", image_field, user_id, str(e))
        raise Http404("Image not found")
...
def vehicle_image(request, vehicle_id, image_field):
    ...
    try:
        logger.debug("[vehicle_image] Attempting to serve %s for vehicle %s", image_field, vehicle_id)
        vehicle = Vehicle.objects.get(id=vehicle_id)
        image_data = getattr(vehicle, image_field)

        logger.debug("[vehicle_image] Image data loaded")
        ...
        logger.debug("[vehicle_image] Serving image content_type=%s", content_type)
        ...
        logger.exception("[vehicle_image] Error serving image %s for vehicle %s: %s", image_field, vehicle_id, str(e))
        raise Http404("Image not found")
...
def user_vehicles(request, user_id):
    try:
        ...
        if request.method == 'POST':
            ...
            if vehicle_type not in [Vehicle.TWO_WHEELER, Vehicle.FOUR_WHEELER]:
                vehicle_type = Vehicle.TWO_WHEELER
            ...
            if fuel_type in [None, '', 'null']:
                fuel_type = ''
            else:
                ft = str(fuel_type).strip().lower()
                fuel_map = {
                    'petrol': 'Petrol',
                    'gasoline': 'Petrol',
                    'diesel': 'Diesel',
                ...
                fuel_type = fuel_map.get(ft, str(fuel_type).strip())
            ...
            vehicle = Vehicle(
                owner=user,
                model_number=str(payload.get('model_number') or payload.get('model') or '').strip(),
                variant=str(payload.get('variant') or '').strip(),
                company_name=str(payload.get('company_name') or payload.get('make') or '').strip(),
                plate_number=plate_number,
                vehicle_type=vehicle_type,
                color=str(payload.get('color') or '').strip(),
                seats=seats_value,
                engine_number=str(payload.get('engine_number') or '').strip(),
                chassis_number=str(payload.get('chassis_number') or '').strip(),
                fuel_type=fuel_type,
...
def vehicle_detail(request, vehicle_id):
    try:
        ...
        if request.method in ['PATCH', 'PUT']:
            ...
            for k in ['model_number', 'company_name', 'variant', 'color', 'engine_number', 'chassis_number']:
                if k in requested_changes and requested_changes.get(k) is not None:
                    requested_changes[k] = str(requested_changes.get(k)).strip()

            if 'vehicle_type' in requested_changes and requested_changes.get('vehicle_type') is not None:
                vt = str(requested_changes.get('vehicle_type')).strip().upper()
                if vt not in [Vehicle.TWO_WHEELER, Vehicle.FOUR_WHEELER]:
                    return JsonResponse({'success': False, 'error': 'Invalid vehicle_type. Use TW or FW.'}, status=400)
                requested_changes['vehicle_type'] = vt

            if 'fuel_type' in requested_changes:
                fuel_raw = requested_changes.get('fuel_type')
                if fuel_raw in [None, '', 'null']:
                    requested_changes['fuel_type'] = ''
                else:
                    ft = str(fuel_raw).strip().lower()
                    fuel_map = {
                        'petrol': 'Petrol',
                        'gasoline': 'Petrol',
                        'diesel': 'Diesel',
                    ...
                    requested_changes['fuel_type'] = fuel_map.get(ft, str(fuel_raw).strip())

            if 'seats' in requested_changes:
                raw_seats = requested_changes.get('seats')
                if raw_seats in [None, '', 'null']:
                    requested_changes['seats'] = None
                else:
                    try:
                        requested_changes['seats'] = int(raw_seats)
                    except Exception:
                        return JsonResponse({'success': False, 'error': 'Invalid seats. Must be an integer.'}, status=400)

            if 'plate_number' in requested_changes and requested_changes.get('plate_number') is not None:
                requested_changes['plate_number'] = str(requested_changes.get('plate_number')).strip().upper()

            next_vehicle_type = requested_changes.get('vehicle_type')
            if next_vehicle_type is None:
                next_vehicle_type = getattr(v, 'vehicle_type', None)

            if next_vehicle_type == Vehicle.TWO_WHEELER:
                requested_changes['seats'] = None
            elif next_vehicle_type == Vehicle.FOUR_WHEELER:
                if 'seats' not in requested_changes:
                    if getattr(v, 'seats', None) is None:
                        return JsonResponse({'success': False, 'error': 'Please specify number of seats for four wheelers.'}, status=400)
