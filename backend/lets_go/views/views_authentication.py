...
def get_user_data_dict(request, user):
    ...
    logger.debug("[get_user_data_dict] built user data payload")
    return data
...
def login(request):
    if request.method == 'POST':
        # Backward compatible: client may send either username OR email in the 'email' field.
        # Also accept an explicit 'username' field.
        identifier = (request.POST.get('email') or request.POST.get('username') or '').strip()
        password = request.POST.get('password')
        logger.debug("[login] incoming identifier=%s", identifier)
        ...
        try:
            ...
            if check_password(password, user.password):
                ...
                return JsonResponse({'success': False, 'error': 'Invalid username/email or password'}, status=404)
            ...
            logger.exception('[login] OperationalError while querying UsersData: %s', repr(e))
    ...
    try:
        ...
        if not user_id and request.body:
            try:
                ...
                logger.exception('[logout_view] Failed to parse body JSON: %s', str(e))

        logger.debug('[logout_view] Resolved user_id=%s', user_id)
        if user_id:
            try:
                updated = UsersData.objects.filter(id=user_id).update(fcm_token=None)
                if updated == 0:
                    logger.debug('[logout_view] No UsersData row updated for id=%s', user_id)
                else:
                    logger.debug('[logout_view] Cleared fcm_token for user %s', user_id)
            except OperationalError as e:
                # Database connection issue; log and continue so logout still succeeds
                logger.exception('[logout_view] OperationalError while clearing fcm_token: %s', repr(e))
            except Exception as e:
                logger.exception('[logout_view][ERROR during fcm_token update]: %s', repr(e))
        try:
            request.session.flush()
        except Exception:
        ...
        logger.exception('[logout_view][ERROR]: %s', repr(e))
        return JsonResponse({'success': False, 'error': str(e)}, status=500)
...
def change_password(request, user_id: int):
    ...
    try:
        ...
        if not current_password or not new_password:
            return JsonResponse({'success': False, 'error': 'current_password and new_password are required'}, status=400)
            ...
            return JsonResponse(
                {
                    ...
                    'fields': {'new_password': [strength_err]},
                },
                status=400,
            )
        ...
        user = UsersData.objects.only('id', 'password').get(id=user_id)
...
def check_username(request):
    ...
    logger.debug("[check_username] username='%s' previous_username='%s'", username, previous_username)

    if not username:
        return JsonResponse({'available': False, 'error': 'Username is required.'})

    # Enforce the same allowed charset as Flutter/Admin to avoid reserving invalid usernames.
    if not re.fullmatch(r"[A-Za-z0-9._]+", username):
        return JsonResponse({'available': False, 'error': 'Only letters, numbers, . and _ are allowed.'})
    ...
    logger.debug("[check_username] reserved username row id=%s", obj.id)
...
def signup(request):
    if request.method == 'POST':
        try:
                ...
                return JsonResponse(
                    {
                        'success': False,
                        'error': 'Validation failed',
                        'fields': {'username': ['Only letters, numbers, . and _ are allowed.']},
                    },
                    status=400,
                )
            ...
            if UsersData.objects.filter(email__iexact=email_norm).exists():
                return JsonResponse({'success': False, 'error': 'Email already registered.'}, status=400)
            if UsersData.objects.filter(username__iexact=username_norm).exists():
            ...
            # Create user
            logger.debug("[signup] creating user username=%s email=%s", username, email)
            ...
            try:
                ...
                logger.exception("Supabase upload error for user images: %s", str(upload_err))
            ...
            user = UsersData(
                ...
                email=email_norm,
                password=make_password((data.get('password') or '').strip()),
                address=(data.get('address') or '').strip(),
                phone_no=phone,
                cnic_no=(data.get('cnic_no') or '').strip(),
                gender=(data.get('gender') or '').strip(),
                driving_license_no=(data.get('driving_license_no') or '').strip(),
                accountno=(data.get('accountno') or '').strip(),
            ...
            if emergency_name and emergency_relation and emergency_email and emergency_phone:
                phone_digits = str(emergency_phone).strip()
            ...
            if vehicles_json:
                ...
                for v in vehicles:
                    ...
                    try:
                        ...
                        logger.exception("Supabase upload error for vehicle %s: %s", str(plate), str(upload_err))
                        return JsonResponse({'success': False, 'error': 'Failed to upload vehicle images. Please try again.'}, status=500)

                    veh = Vehicle(
                        owner=user,
                        model_number=str(v.get('model_number', '') or '').strip(),
                        variant=str(v.get('variant', '') or '').strip(),
                        company_name=str(v.get('company_name', '') or '').strip(),
                        plate_number=str(plate or '').strip().upper(),
                        vehicle_type=str(v.get('vehicle_type', 'TW') or 'TW').strip().upper(),
                        ...
                        engine_number=str(v.get('engine_number', '') or '').strip(),
                        chassis_number=str(v.get('chassis_number', '') or '').strip(),
                        fuel_type=str(v.get('fuel_type', '') or '').strip(),
            ...
            try:
                UsernameRegistry.objects.filter(username__iexact=username_norm).delete()
            except Exception:
                pass
...
def send_otp(request):
    ...
    if request.method == 'POST':
        ...
        logger.debug("[send_otp] request otp_for=%s resend=%s email_present=%s phone_present=%s", otp_for, resend, bool(email), bool(phone))
        ...
        if otp_for == 'registration':
            if email and cache_data.get('email_otp'):
                send_email_otp(email, cache_data['email_otp'])
            if phone and cache_data.get('phone_otp'):
...
def verify_password_reset_otp(request):
    if request.method == 'POST':
        ...
        logger.debug("[verify_password_reset_otp] method=%s", method)
        ...
        cache_key = get_reset_cache_key(method, value)
        logger.debug("[verify_password_reset_otp] cache_key=%s", cache_key)
        cached = cache.get(cache_key)
        logger.debug("[verify_password_reset_otp] cached=%s", cached)
        if not cached:
                    ...
                    if not re.search(r"[A-Z]", new_password or '')
                        ...
                        if not re.search(r"[a-z]", new_password or '')
                            ...
                            if not re.search(r"\d", new_password or '')
                                ...
                                if not re.search(r"[!@#$%^&*()_+\-=\[\]{};':\"\\|,.<>/?]", new_password or '')
                                else None
                            )
                        )
                    )
