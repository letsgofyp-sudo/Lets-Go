...
def sos_incident(request):
    ...
    dedupe_window_seconds = _coerce_int(os.getenv('SOS_DEDUPE_WINDOW_SECONDS', '60')) or 60
    if dedupe_window_seconds > 0:
        try:
            since = timezone.now() - timedelta(seconds=dedupe_window_seconds)
            existing = (
                SosIncident.objects.filter(
                    trip=trip,
                    actor=actor,
        ...
        if existing is not None:
            ...
            share_token = SosShareToken.mint(incident=existing, expires_at=expires_at)
            share_url = request.build_absolute_uri(
                reverse('sos_share', kwargs={'token': share_token.token})
            )

            try:
                e_lat = float(existing.latitude)
                e_lng = float(existing.longitude)
            except Exception:
                e_lat = lat
                e_lng = lng

            maps_url = f"https://www.google.com/maps?q={e_lat},{e_lng}"

            return JsonResponse({
                'success': True,
                'deduped': True,
                'audit_event_id': getattr(existing.audit_event, 'id', None),
                'incident_id': existing.id,
                'share_url': share_url,
                'maps_url': maps_url,
                'notified': {
                    'emergency_contact_email': False,
                    'emergency_contact_sms': False,
                    'admin_email': False,
                },
            })
    ...
    if emergency is not None:
        if emergency.email:
            email_to_contact_ok = send_incident_email([emergency.email], subject, body)
            logger.debug("[sos_incident] sent email to emergency contact")
        if emergency.phone_no:
            sms_to_contact_ok = send_incident_sms(emergency.phone_no, body)
            logger.debug("[sos_incident] sent sms to emergency contact")

    admin_email_ok = send_incident_email([admin_email], subject, body)
...
def trip_share_view(request, token):
    ...
    try:
        ...
        if route is not None:
            ...
            pts = list(RouteGeometryPoint.objects.filter(route=route).only('latitude', 'longitude').order_by('point_index'))
            if pts:
                for p in pts:
                    route_geometry_data.append({'lat': float(p.latitude), 'lng': float(p.longitude)})
            else:
