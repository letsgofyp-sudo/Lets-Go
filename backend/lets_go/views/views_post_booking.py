...
from ..models.models_route import RouteStop, RouteGeometryPoint
...
from ..models.models_incident import TripShareToken
from ..utils.email_otp import send_incident_email
from ..utils.phone_otp_send import send_incident_sms
...
def _send_email(subject: str, body: str, recipients: list[str]) -> bool:
    return send_incident_email(recipients, subject, body)
...
def _compute_reached_trigger_dt(trip: Trip) -> tuple[datetime | None, float | None]:
    ...
    return trigger_dt, delay_hours


def _mint_share_url(request, trip: Trip, role: str, booking: Booking | None = None) -> str | None:
    try:
        expires_at = timezone.now() + timedelta(hours=6)
        share_token = TripShareToken.mint(trip=trip, role=role, booking=booking, expires_at=expires_at)
        return request.build_absolute_uri(reverse('trip_share', kwargs={'token': share_token.token}))
    except Exception:
        return None


def _require_cron_secret(request):
    expected = os.getenv('CRON_SECRET') or os.getenv('SUPABASE_CRON_SECRET')
    if not expected:
        return None
    got = (request.headers.get('x-cron-secret') or request.headers.get('X-CRON-SECRET') or '').strip()
...
def cron_post_booking_reached_reminders(request):
    auth_err = _require_cron_secret(request)
    ...
    trips = (
        ...
        .only('id', 'trip_id', 'trip_date', 'departure_time', 'estimated_arrival_time', 'trip_status', 'driver_id')
    ...
    for trip in trips[:400]:
        trigger_dt, delay_hours = _compute_reached_trigger_dt(trip)
        ...
        if trip.trip_status != 'COMPLETED':
            ...
            if _record_system_notification_if_due(trip, key, cooldown_seconds=86400 * 7):
                ...
                payload_driver = {
                    ...
                    'title': 'Reached reminder',
                    'body': 'If you have reached, please mark as reached in the app.',
                    'data': {
                        'type': 'reached_reminder_driver',
        ...
        for b in bookings:
            ...
            if ride_status == 'DROPPED_OFF':
                continue

            key = f"reached_reminder_passenger_{b.id}"
            if not _record_system_notification_if_due(trip, key, cooldown_seconds=86400 * 7):
                continue

            share_url = _mint_share_url(request, trip, role='passenger', booking=b)
            payload_passenger = {
                'user_id': str(b.passenger_id),
                'driver_id': str(trip.driver_id),
                'title': 'Reached reminder',
                'body': 'If you have reached, please mark as reached in the app.',
                'data': {
                    'type': 'reached_reminder_passenger',
                    'trip_id': str(trip.trip_id),
                    'booking_id': str(b.id),
                },
            }
    ...
    return JsonResponse({
        ...
        'sent': {
            'driver_push': sent_driver,
            'passenger_push': sent_passengers,
...
def start_trip_ride(request, trip_id):
    ...
    # A driver cannot start a new ride while they are already in an active trip.
    try:
        already_active = (
            Trip.objects
            .filter(driver_id=driver_id, trip_status='IN_PROGRESS')
            .exclude(id=trip.id)
            .exists()
        )
    except Exception:
    ...
    if already_active:
        return JsonResponse({'success': False, 'error': 'You already have an active trip in progress.'}, status=400)
...
def start_booking_ride(request, booking_id):
    ...
    # A passenger cannot start a new ride while they are already on-board in another booking.
    try:
        already_on_board = (
            Booking.objects
            .filter(passenger_id=passenger_id, ride_status='RIDE_STARTED')
...
def complete_trip_ride(request, trip_id):
    ...
    # Intentionally do not send a generic "Trip completed" push to passengers.
    # Passenger flow already ends via drop-off -> payment -> rating.
...
def confirm_booking_payment(request, booking_id):
    ...
    try:
        ...
        payload = {
            ...
            'body': f'Your payment for booking {booking.id} was confirmed by the driver.',
            'data': {
...
def get_live_location(request, trip_id):
    ...
    # Driver should not see passengers who already completed their individual ride.
    if requester_role == 'DRIVER' and isinstance(live_state, dict):
        try:
            passengers = live_state.get('passengers')
            if isinstance(passengers, list) and passengers:
                ...
                if booking_ids:
                    active = (
                        Booking.objects
                        .filter(trip=trip, id__in=booking_ids, ride_status='RIDE_STARTED')
                        .only('id')
                    )
                    allowed_ids = set([b.id for b in active])

                if allowed_ids is not None:
                    live_state = dict(live_state)
                    live_state['passengers'] = [
                        p for p in passengers
                        if isinstance(p, dict) and p.get('booking_id') in allowed_ids
                    ]
        except Exception:
            pass
        ...
        if isinstance(driver_obj, dict) and driver_obj.get('lat') is not None and driver_obj.get('lng') is not None:
            ...
            try:
                ...
                if route is not None:
                    pts = list(
                        RouteGeometryPoint.objects
                        .filter(route=route)
                        .only('latitude', 'longitude')
                        .order_by('point_index')
                    )
                if pts:
                    geom = [{'lat': float(p.latitude), 'lng': float(p.longitude)} for p in pts]
                else:
                    geom = getattr(route, 'route_geometry', None) if route is not None else None
            except Exception:
                geom = None
...
def compute_driver_reminder_time(trip: Trip):
    ...
    logger.debug(
        "[compute_driver_reminder_time] trip_id=%s trip_date=%s departure_time=%s trip_dt=%s trigger_at=%s",
        trip.trip_id,
        trip.trip_date,
        trip.departure_time,
        trip_dt.isoformat(),
...
def compute_passenger_reminder_time(trip: Trip, from_stop_order: int):
    ...
    try:
        ...
        if total_minutes > 0:
            ...
            logger.debug(
                "[compute_passenger_reminder_time] trip_id=%s from_stop_order=%s via TripStopBreakdown total_minutes=%s pickup_eta=%s trigger_at=%s",
                trip.trip_id,
                from_stop_order,
                total_minutes,
                pickup_eta.isoformat(),
                trigger_at.isoformat(),
            )
            return trigger_at
    except Exception:
    ...
    try:
        ...
        if getattr(stop, 'estimated_time_from_start', None) is not None:
            ...
            trigger_at = pickup_eta - timezone.timedelta(minutes=10)
            logger.debug(
                "[compute_passenger_reminder_time] trip_id=%s from_stop_order=%s via RouteStop est_minutes=%s pickup_eta=%s trigger_at=%s",
                trip.trip_id,
                from_stop_order,
                stop.estimated_time_from_start,
                pickup_eta.isoformat(),
                trigger_at.isoformat(),
            )
            return trigger_at
...
def build_pre_ride_reminder_jobs_for_trip(trip: Trip):
    ...
    for booking in bookings:
        ...
        if from_order is None:
            logger.debug(
                "[build_pre_ride_reminder_jobs_for_trip] trip_id=%s booking_id=%s has no from_stop_order, skipping passenger job",
                trip.trip_id,
                booking.id,
            )
            continue
    ...
    logger.debug(
        "[build_pre_ride_reminder_jobs_for_trip] trip_id=%s driver_trigger_at=%s passenger_jobs_count=%s",
        trip.trip_id,
        driver_trigger_at.isoformat(),
        len(passenger_jobs),
    )
...
def fire_pre_ride_reminder_notifications(trip: Trip):
    ...
    try:
        ...
        logger.debug(
            "[fire_pre_ride_reminder_notifications] Sending DRIVER reminder trip_id=%s driver_id=%s",
            trip.trip_id,
            trip.driver_id,
        )
        send_ride_notification_async(driver_payload)
    except Exception as e:
        logger.exception('[fire_pre_ride_reminder_notifications][driver][ERROR]: %s', str(e))
    ...
    for booking in bookings:
        try:
            ...
            logger.debug(
                "[fire_pre_ride_reminder_notifications] Sending PASSENGER reminder trip_id=%s booking_id=%s passenger_id=%s",
                trip.trip_id,
                booking.id,
            ...
            logger.exception('[fire_pre_ride_reminder_notifications][passenger][ERROR]: %s', str(e))
