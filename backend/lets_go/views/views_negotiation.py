    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                logger.exception("[handle_ride_booking_request] Error normalizing fare fields: %s", str(e))
                original_fare = int(trip.base_fare or 0)
                final_fare = int(original_fare)
            ...
            try:
                ...
                logger.debug("[handle_ride_booking_request] driver_user_id=%s", driver_user_id)
                if driver_user_id:
                    payload = {
                ...
                logger.exception("[handle_ride_booking_request][notify_error]: %s", str(e))
...
def list_pending_requests(request, trip_id):
    ...
    try:
        t0 = pytime.time()
        logger.debug("[list_pending_requests] START trip_id=%s", trip_id)
        ...
        logger.debug("[list_pending_requests] Trip lookup took %.1fms (pk=%s)", (pytime.time() - t1) * 1000.0, trip_pk)
        ...
        logger.debug("[list_pending_requests] Requests query took %.1fms", (pytime.time() - t2) * 1000.0)
        ...
        logger.debug(
            "[list_pending_requests] Serialize took %.1fms, total elapsed %.1fms",
            (pytime.time() - t3) * 1000.0,
            (pytime.time() - t0) * 1000.0,
        )
        ...
        try:
            logger.exception('[list_pending_requests] OperationalError, attempting reconnect: %s', str(e))
            connection.close()
            connection.connect()
            # Retry logic
            trip_row = (
            ...
            logger.exception('[list_pending_requests][RETRY_FAIL]: %s', str(ex))
            return JsonResponse({'success': False, 'error': 'Database connection error, please retry'}, status=500)
    except Exception as e:
        logger.exception('[list_pending_requests][ERROR]: %s', str(e))
...
def booking_request_details(request, trip_id, booking_id):
    ...
    try:
        ...
        logger.exception('[RESPOND_REQUEST][ERROR] %s', str(e))
        return JsonResponse({'success': False, 'error': str(e)}, status=500)
...
def respond_booking_request(request, trip_id, booking_id):
    ...
    try:
        t0 = pytime.time()
        logger.debug("[respond_booking_request] START trip_id=%s booking_id=%s", trip_id, booking_id)
        ...
        logger.debug(
            "[respond_booking_request] Parsed payload action=%s driver_id=%s counter_fare=%s",
            action,
            driver_id,
            counter_fare,
        )
        ...
        logger.debug("[respond_booking_request] Loaded trip in %.1fms driver_id=%s", (pytime.time() - t1) * 1000.0, trip.driver_id)
        ...
        if action == 'accept':
            ...
            try:
                ...
                pass
            logger.debug(
                "[respond_booking_request] ACCEPT branch completed in %.1fms",
                (pytime.time() - t3) * 1000.0,
            )
            ...
            try:
                ...
                logger.exception('[respond_booking_request][notify_error][accept]: %s', str(e))
            logger.debug("[respond_booking_request] END action=accept total_elapsed=%.1fms", (pytime.time() - t0) * 1000.0)
            ...
            try:
                ...
                pass
            logger.debug("[respond_booking_request] REJECT branch completed in %.1fms", (pytime.time() - t3) * 1000.0)
            ...
            try:
                ...
                logger.exception('[respond_booking_request][notify_error][reject]: %s', str(e))
            logger.debug("[respond_booking_request] END action=reject total_elapsed=%.1fms", (pytime.time() - t0) * 1000.0)
            ...
            try:
                ...
                logger.exception('[respond_booking_request][notify_error][counter]: %s', str(e))
...
def passenger_respond_booking(request, trip_id, booking_id):
    ...
    try:
        t0 = pytime.time()
        logger.debug("[passenger_respond_booking] START trip_id=%s booking_id=%s", trip_id, booking_id)
        data = json.loads(request.body or '{}')
        action = (data.get('action') or '').lower()
        passenger_id = data.get('passenger_id')
        counter_fare = data.get('counter_fare')
        ...
        logger.debug(
            "[passenger_respond_booking] Parsed payload action=%s passenger_id=%s counter_fare=%s",
            action,
            passenger_id,
            counter_fare,
        )
        ...
        logger.debug(
            "[passenger_respond_booking] Loaded trip and booking in %.1fms booking.passenger_id=%s",
            (pytime.time() - t1) * 1000.0,
        ...
        if action == 'accept':
            ...
            logger.debug(
                "[passenger_respond_booking] ACCEPT branch: loaded trip for seat check in %.1fms available_seats=%s",
                (pytime.time() - t2) * 1000.0,
                t.available_seats,
            )
            ...
            logger.debug(
                "[passenger_respond_booking] ACCEPT branch: updated booking and seats in %.1fms",
                (pytime.time() - t2) * 1000.0,
            )
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][history_error][accept]: %s', str(e))
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][notify_error][accept]: %s', str(e))
            logger.debug("[passenger_respond_booking] END action=accept total_elapsed=%.1fms", (pytime.time() - t0) * 1000.0)
            return JsonResponse({'success': True, 'message': 'Booking confirmed', 'booking': {
                'id': booking.id,
                'status': booking.booking_status,
                'bargaining_status': booking.bargaining_status,
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][history_error][counter]: %s', str(e))
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][notify_error][counter]: %s', str(e))
            logger.debug("[passenger_respond_booking] END action=counter total_elapsed=%.1fms", (pytime.time() - t0) * 1000.0)
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][history_error][withdraw]: %s', str(e))
            ...
            try:
                ...
                logger.exception('[passenger_respond_booking][notify_error][withdraw]: %s', str(e))
            logger.debug("[passenger_respond_booking] END action=withdraw total_elapsed=%.1fms", (pytime.time() - t0) * 1000.0)
        ...
        logger.exception('[PASSENGER_RESPOND][ERROR] %s', str(e))
        return JsonResponse({'success': False, 'error': str(e)}, status=500)
    ...
    if request.method == 'POST':
        try:
            ...
            with transaction.atomic():
                ...
                t2 = timezone.now()
                logger.debug(
                    "[request_ride_booking] Trip lock fetch %.1fms",
                    (t2 - t1).total_seconds() * 1000.0,
                )
            ...
            try:
                driver_user_id = getattr(trip, 'driver_id', None)
                logger.debug("[request_ride_booking] driver_user_id=%s", driver_user_id)
                if driver_user_id:
                    payload = {
                        'user_id': str(driver_user_id),
                        'driver_id': str(driver_user_id),
                ...
                logger.exception('[request_ride_booking][notify_error]: %s', str(e))

            t3 = timezone.now()
            logger.debug(
                "[request_ride_booking] Total elapsed %.1fms",
                (t3 - t0).total_seconds() * 1000.0,
            )
            ...
            logger.exception('[request_ride_booking][DB_ERROR]: %s', str(e))
