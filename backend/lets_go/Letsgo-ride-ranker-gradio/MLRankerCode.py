...
def root():
    return {
        ...
        "endpoints": {
            "rank": "/",
            "rank_compat": "/rank-rides",
            "rank_compat2": "/api/rank-rides",
            "health": "/health",
        },
    }
...
def _parse_iso_dt(s: Optional[str]) -> Optional[datetime]:
    """
    Parse ISO datetime string into a Karachi timezone-aware datetime.
    - If datetime has no timezone → assume Karachi.
    - If datetime has timezone → convert to Karachi.
    """
    if not s:
        return None
...
@app.post("/rank-rides", response_model=RankResponse)
def rank_rides_root(req: RankRequest, authorization: Optional[str] = Header(default=None)):
    return rank_rides(req=req, authorization=authorization)


@app.post("/", response_model=RankResponse)
def rank_rides_slash(req: RankRequest, authorization: Optional[str] = Header(default=None)):
    return rank_rides(req=req, authorization=authorization)